...
num_folds = 5
n_classes = 2
in_chans = 12  # train_set.X.shape[1]  # number of channels = 12
input_time_length = 1000  # length of time of each epoch/trial = 4000
...
################### Load data
idx_healthy, idx_patient, CV_healthy_train, CV_healthy_test, CV_patient_train, CV_patient_test = dataset.get_PTB(n_splits=num_folds)
...
for CV in range(num_folds): #Cross-validation
    ...
    for i_epoch in range(n_epochs):  # Need to change
        ...
        for i in range(len(test_data_path)):  # Test for each test subject
            ...
            FeatVect = np.expand_dims(ECG_unfiltered, 0)
